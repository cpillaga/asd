...
getExpPuesto(idP: string){
  ...
  const url = URL_SERVICE.url + '/expPuestobusc/'+this.idEmp+ "/" + idP ;
...
updExpPuest(area: string, tiempo: string, especificidad: string, experiencia: string, puesto: string, idexpPuesto: string){
  let token = localStorage.getItem('tokenFact');
  const headers = new HttpHeaders({
    token
  });

  const url = URL_SERVICE.url + '/expPuesto/' + idexpPuesto;
...
updExpPuestTiempEsp( id: string, tiempo: string, especificidad: string ){
  let token = localStorage.getItem('tokenFact');
  const headers = new HttpHeaders({
    token
  });
